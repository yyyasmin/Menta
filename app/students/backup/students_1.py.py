...
def student_add():
    ...
    new_profile = Profile(title='New profile', body='No description yet', author_id=author_id.id)
...
def plan_report():
    ...
    std_goals = Goal.query.filter(Goal in std.general_txts).all()
...
def edit_students_profile():
	...
	profile = Profile.query.filter(Profile in student.general_txts).first()
	if profile:
...
def edit_students_profile2(selected_student_id):
	...
	prf = Profile.query.filter(Profile in student.general_txts).first()
...
def edit_std_profile_subjects():
    ...
    std_subjects = Subject.query.filter(Subject in std.general_txts).all()
    #############import_pdb; pdb.set_trace()
...
def subject_to_std_profile_add():
    ...
    prof_subjects = Subject.query.filter(Subject in profile.children).all()
    subjects_not_of_student = list(set(all_subjects).difference(set(prof_subjects)))  #subjects_not_of_student = all_subjects-student's subjects
...
def edit_std_profile_weaknesses():
    ...
    std_weaknesses = Weakness.query.filter(Weakness in profile.children).all()
    #############import_pdb; pdb.set_trace()
...
def weakness_to_std_profile_add():
    ...
    prof_weaknesses = Weakness.query.filter(Weakness in profile.children).all()
...
def edit_std_profile_strengthes():
    ...
    std_strengthes = Strength.query.filter(Strength in profile.children).all()
    #############import_pdb; pdb.set_trace()
...
def strength_to_std_profile_add():
    ...
    prof_strengthes = Strength.query.filter(Strength in profile.children).all()
    strengthes_not_of_student = list(set(all_strengthes).difference(set(prof_strengthes)))  #strengthes_not_of_student = all_strengthes-student's strengthes
...
def edit_student_destinations():
    ...
    #DEBUG
    std_dst_ids = []
    for st in std.general_txts:
        if st.type == 'destination':
            std_dst_ids.append(st.id)
...
def destination_to_student_add():
    ...
    ##################import_pdb; pdb.set_trace()
    #destinations_of_std_and_dummy = Std_general_txt.query.filter(Std_general_txt.student_id==0).filter(Std_general_txt.id.in_(std_dst_ids)).filter(Std_general_txt.type=='destination').all()
    #all_destinations = Std_general_txt.query.filter(Std_general_txt.student_id==0).filter(Std_general_txt.type=='destination').all()   # Humpty dumpty has id=0 and all destinations 
    destinations_of_std_and_dummy = Destination.query.filter(Destination in std.general_txts).all()
...
def match_destination_to_student():
    ...
    std_dst.student = std
    std_dst.general_txt = dst
    std.general_txts.append(std_dst)
    dst.students.append(std_dst)

    for tag in dst.general_choices :
        if tag.type == 'tag':
            #std_tag = Std_tag.query.filter(Std_tag.student_id==std.id).filter(Std_tag.tag_id==tag.tag_id).first()        if std_tag == None:
            std_tag = Std_general_choice.query.filter(Std_general_choice.student_id==std.id).filter(Std_general_choice.general_choice_id==tag.id).first()     
            if std_tag == None:
                std_tag = Std_general_choice(std.id, tag.id)
                std_tag.student = std
                std_tag.general_choice = tag            
                std.general_choices.append(std_tag)
                tag.students.append(std_tag)
...
def destination_from_student_delete():
	...
	student.general_txts.remove(destination)
	db.session.commit()  
...
def try_get_todos_not_of_student():
    ...
    #all_goal_todos = Todo.query.filter(Todo in std_goal.children).all()
    all_goal_todos = Todo.query.filter(Todo in std_goal.children)
