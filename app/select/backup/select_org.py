...
from app.models import Std_goal, Std_resource, Std_todo, Std_document
...
@slct.route('/teacher_select', methods=['GET', 'POST'])
def teacher_select():
	#print("1111111111111")
...
@slct.route('/student_select', methods=['GET', 'POST'])
def student_select():
	#print("1111111111111")
	...
	if request.method == 'GET':
		return render_template('show_students3.html', students=students)
...
@slct.route('/profile_select', methods=['GET', 'POST'])
def profile_select():
	#print("1111111111111")
	...
	if request.method == 'GET':
		return render_template('profiles.show_teachers.html', profiles=profiles)
	...
	#print(teacher.selected)
	profiles = Profile.query.all()
	return render_template('profile.show_selected_profile.html', profle=profile)
...
@slct.route('/destination_select', methods=['GET', 'POST'])
def destination_select():
    #print("In destination_select 1111111111111")
    ...
    if request.method == 'GET':
        return render_template('destinations.edit_destinations.html', destinations=destinations)
        
        
    selected_destination_id = int(request.form['selected_destination'])

    destination = Destination.query.get_or_404(selected_destination_id)		
    ...
    destinations = Destination.query.all()
    #return render_template('show_selected_destination.html', destinations=destinations)
...
def std_goal_select2(selected_std_id, selected_goal_id):
    ...
    std_goal = Std_goal.query.filter(Std_goal.goal_id == selected_goal_id).filter(Std_goal.student_id==selected_std_id).first()
    if std_goal == None:
        flash("Please select a goal for student ")
        return redirect(url_for("students.edit_student_goals"))
...
def subject_select2(selected_subject_id):
	...
	sbj = Subject.query.get_or_404(selected_subject_id)
