...
def dsply_gt_form_for_add(type): 
    ...
    if request.method == 'GET':
        return render_template('./gt/dsply_gt_form.html', form=form)
...
def gt_to_profile_add(type):
    ...
    gt_type =      form.gt_type
    
    return gt_to_profile_add(tag.title, new_gt_title, new_gt_body, gt_type)
    ...
    for c in new_gt.children.all():
        print("CCCCCCCCCCC IN gt_to_profile_add c: c.id:   ", c, c.id)
    #DEBUG
