...
def edit_star_destinations(selected_star_id):
    ...
    print("IN edit_star_destinations SELECTED-STAR-ID: ", selected_star_id)

    star = star_select2(selected_star_id)
    print("")
    print("")
    print("STAR: ", star.id, star.body)
    
    
    tags=[]
    tag = Tag.query.filter(Tag.title == star.title).first()    #get only Tag destinations
    if tag == None:
        flash ("אין ערך זה בבנק. ")
    ...
    for dst in all_dsts:
        if dst in tag.children:
            tag_dsts.append(dst)
    if tag_dsts == []:
        flash("אין מטרות לערך זה. נא להוסיף מטרה דרך לשונית מטרות שבסרגל הכתום שלמעלה")
        return redirect(url_for('stars.star'))   
    ...
    return render_template('edit_dst_by_subject.html', destinations=all_dsts, age_ranges=age_ranges, tags=tags, sub_tags=sub_tags)															
